...
const functions = {
	...
	async getInspectorTree(payload) {
		const res = await __vue_devtools_kit.devtools.ctx.api.getInspectorTree(payload);
		return (0, __vue_devtools_kit.stringify)(res);
	},
	async getInspectorState(payload) {
		...
		const res = await __vue_devtools_kit.devtools.ctx.api.getInspectorState(payload);
		return (0, __vue_devtools_kit.stringify)(res);
	},
	...
	initDevToolsServerListener() {
		const rpcServer$1 = (0, __vue_devtools_kit.getRpcServer)();
		const broadcast = rpcServer$1.broadcast;
